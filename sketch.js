...
function play(){
  ...
  if(frameCount % 200 === 0){
    ...
    switch(rand){
      ...
      break;
      //case 4 : obstacle.addImage("egypt",egyptImg);
      //break;
      default : break;
    }
  ...
  if(frameCount%1000 === 0){
    jin = createSprite(displayWidth,displayHeight-500);
    jin.addImage("puzzle",jinImg);
    jin.mirrorX(-1);
    jin.scale = 0.8;
    jin.velocityX = -4;
    jGroup.add(jin);
...
function pause(){
  puzzle = createSprite(displayWidth/2+3,displayHeight/2+3,1,1);
  oGroup.destroyEach();
  path.velocityX = 0;
  jGroup.destroyEach();
  dGroup.destroyEach();
  boy.destroy();
  if(frameCount >500 && frameCount< 1500){
    puzzle.addImage("puzzle1",p1);
    if()
  }
  if(frameCount >= 2000 && frameCount <=2500){
    puzzle.addImage("puzzle2",p2);
  }
  if(frameCount >= 2500 && frameCount <=3000){
    puzzle.addImage("puzzle3",p3);
  }
  drawSprites();
}